...
""",
    ...
    p.add_argument(
        ...
        nargs="?",
        default="",
        help="optional subcommand: start|stop|restart|status|once|doctor|init",
    )
    return p
...
def cmd_setup_wifi(cfg) -> int:
    ...
    adb.start_server()
    try:
        dev = adb.wait_online(timeout=60, on_pending=lambda m: print(f"fiver: {m}"))
        ...
        print(f"fiver: wireless ADB ready at {target}")
        print("fiver: unplug USB if you want, then: fiver --start")
        return 0
    except ADBError as exc:
        print(f"fiver: {exc}", file=sys.stderr)
        return 1
